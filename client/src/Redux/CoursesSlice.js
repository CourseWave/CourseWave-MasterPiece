  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(addCourseSection.fulfilled, (state, action) => {
        ...
        // For example, update state.Courses with the new data
      })
...
export const addCourseVideo = createAsyncThunk(
  ...
  async ({ course_section_id, videoData }) => {
    const response = await axios.post(`http://localhost:5000//addCourseVideos/${course_section_id}`, {
      videos: videoData,
    });
    return response.data;
  }
