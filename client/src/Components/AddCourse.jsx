...
import { addVideosToSections, createCourse } from "../Redux/CoursesSlice";
...
const AddCourse = () => {
  ...
  const handleAddVideoInSection = (sectionIndex) => {
    setVideosData((prevData) => {
      ...
      // Check if there are less than 2 videos, add a new video
      if (currentSection.videos.length < 2) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentStep === 1) {
      ...
      try {
        setLoading(true); // Set loading state while submitting
        await dispatch(addVideosToSections(sectionData));
        setCourseData({
          title: "",
          course_catagory: "",
          objectives: [],
